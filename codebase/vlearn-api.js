...
// ---- Slide THẬT từ data pack ------------------------------------------------
// `slides-index.json` do `python codebase/ingest_slides.py` sinh ra từ
// data/vlearn-pack/slides/*.pdf. Có file thì nạp thêm hai bộ slide thật vào
// sidebar; không có thì ứng dụng vẫn chạy y như cũ với dữ liệu mô phỏng.
let SLIDE_THAT_DA_NAP = false;

async function napSlideThat() {
  if (SLIDE_THAT_DA_NAP) return;
  SLIDE_THAT_DA_NAP = true;
  try {
    const res = await fetch("./slides-index.json", { cache: "no-cache" });
    if (!res.ok) return;
    const muc = await res.json();
    const ids = [];
    for (const [mid, info] of Object.entries(muc)) {
      MATERIALS[mid] = {
        id: mid,
        name: info.ten,
        pages: info.so_trang,
        anhThat: info.thu_muc_anh, // vd "slides/material_d1_hackathon"
      };
      ids.push(mid);
    }
    if (ids.length) {
    ...
    /* không có file index -> bỏ qua, dùng dữ liệu mô phỏng */
  }
}
...
export async function get(path) {
  ...
  if ((m = path.match(/^\/materials\/([^/]+)\/pages\/(\d+)$/))) {
    ...
    // Slide thật: trả về ảnh trang đã kết xuất từ PDF.
    if (mat.anhThat) {
      return {
        page: n,
        total: mat.pages,
        kind: "image",
        src: `${mat.anhThat}/${n}.webp`,
        title: mat.name.replace(/\.pdf$/, ""),
      };
    }
